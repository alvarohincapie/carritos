...
public class ServiceRegistryImp implements ServiceRegistryService {
	...
	public void serviceRegistryFuel(Long idFuelTypeRequest, String placa,
		...
		try {
			...
			log.info("Inicia el algoritmo a seguir");
			...
			if (idFuelTypeRequest != null) {
				...
				vehicles = SearchVehicles
						.consultarVehiculosPorPlacaSinFiltros(placaDip);
				if (vehicles != null) {
					assignation = SearchVehicles
							.consultarAsignacionVehiculo(placaDip);
				}
				if (cargoA != null)
				...
				if (idFuelTypeRequest.longValue() == VEHICULO_CIAT
					...
					tariffs = new ConsultTariff()
							.consultarTarifaTipoCombustible(fuelType);
				else
					tariffs = new ConsultTariff()
							.consultarTarifaTipoCombustibleCali(fuelType);
	...
	throws GWorkException,
		...
		try {
			...
			total = new Float(Util.redondear(total, 3));
			...
			if (usoDisponible != null
					&& usoDisponible.longValue() == ParametersUtil.USO_PRESUPUESTO
					&& ((chargeTo.getChtCodigo().longValue() == ParametersUtil.CARGO_A_TERCEROS || chargeTo
							.getChtCodigo().longValue() == ParametersUtil.CARGO_A_CC) && fuelTypeRequest
							.getFtrCodigo().longValue() != ParametersUtil.SOLICITUD_COMB_OTROS)) {

				Long tipoCargo = null;
				Period period = FlatFileFuelServiceImpl
						.consultarPeriodoByNovedad(fechaActual,
				...
				connection = new BudgetFuelProofServiceImpl()
						.generarComprobanteDetalle(connection, carneAsignatario,
								ParametersUtil.COMPRANTE_COMBUSTIBLE, login,
								total, ParametersUtil.CREDITO, null, tipoCargo,
								placa, fechaActual, numeroGalones,
								fuelTypeRequest, period, headerProof);
			}
			// EntityManagerHelper.getEntityManager().getTransaction().begin();
			if (headerProof != null)
			...
			System.out.println("ADMINISTRADOR: " + rolls.getRlsMail());
			String emailCopia = "";
